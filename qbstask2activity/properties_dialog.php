...
<tr>
    ...
    <tr>
        <td align="right" width="40%" valign="top"><?= GetMessage("TASKS_BP_REQUIRE_RESULT") ?>:</td>
        <td width="60%" valign="top">
            <select name="REQUIRE_RESULT">
                <option value="Y"<?= ("Y" == $arCurrentValues["REQUIRE_RESULT"] ? ' selected' : '') ?>><?= GetMessage("BPCGHLP_YES") ?></option>
                <option value="N"<?= ("N" == $arCurrentValues["REQUIRE_RESULT"] ? ' selected' : '') ?>><?= GetMessage("BPCGHLP_NO") ?></option>
            </select>
        </td>
    </tr>
<?php
