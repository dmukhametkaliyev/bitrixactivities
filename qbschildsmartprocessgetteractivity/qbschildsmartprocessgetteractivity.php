...
class CBPQBSChildSmartprocessGetterActivity extends CBPActivity
{
    ...
    public function Execute()
    {
       ...
       $factory = Service\Container::getInstance()->getFactory($this->child_id);
        ...
        $object_ids = array(); // создаем пустой массив для хранения значений "PARENT_ID_164"

        foreach ($items as $item) {
            if (isset($item[$parentDynamicID])) { // проверяем, существует ли поле "PARENT_ID_164" в текущем объекте
                $object_ids[] = $item['ID']; // добавляем значение поля в массив $parent_ids
            }
        }
        $object_ids_string = implode(',', $object_ids);
        //$this->Result = $object_ids_string;
        $this->Result = $object_ids;
